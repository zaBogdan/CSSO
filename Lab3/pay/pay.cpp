...
#define PAYMENTS_DIR "C:\\Facultate\\CSSO\\Laboratoare\\Week3\\Input\\payments\\"
...
bool write_to_normal_file(const char* path, const char* data, int input_size) {
	...
	CHECK(h != INVALID_HANDLE_VALUE, false, "[WriteToFileFile] Failed to create a file %s. Error: %d",path, GetLastError());

	DWORD bytesWritten;

	CHECK(WriteFile(h, data, input_size+1, &bytesWritten, NULL), false, "[WriteToFile] Failed to write to file!. Error: %d", GetLastError());
	CloseHandle(h);
	return true;
}
...
bool update_counts(std::string path, unsigned long long total_size_per_day, bool add = true)
{
	...
	CHECK(read_from_file(path.c_str(), old_value), false, "Failed to read from payments.txt");
	...
	std::string new_value = "5";
		//std::to_string(std::stoi(old_value) + sum);

	CHECK(write_to_normal_file(path.c_str(), new_value.c_str(), new_value.length()),
		false,
		"Failed to write to payments.txt");
	return true;
}
...
bool update_summary(unsigned long long total_size_per_day)
{
	...
	update_counts("C:\\Facultate\\CSSO\\Laboratoare\\Week3\\Reports\\Summary\\summary.txt", total_size_per_day, false);
#ifdef SINGLE_RUN
...
void* log_data_callback(WIN32_FIND_DATA fdd, void* ctx) {
	...
	while (!ss.eof()) {
		...
		word.pop_back();
		std::string buff = "S-a facut o plata de " + word + " in data de " + fdd.cFileName +"\n";
	...
	CHECK(update_counts("C:\\Facultate\\CSSO\\Laboratoare\\Week3\\Reports\\Summary\\payments.txt", total_size_per_day), (void*)false, "Failed to update payments.txt");
